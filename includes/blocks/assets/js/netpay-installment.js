...
const settings = getSetting( 'netpay_installment_data', {} )
...
const InstallmentPaymentMethod = (props) => {
    ...
    const noPaymentMethods = __( 'Purchase Amount is lower than the monthly minimum payment amount.', 'netpay' );
    ...
    return (<>
        {description && <p>{description}</p>}
        {
            installment_backends.length == 0
                ...
                : (
                    <fieldset id="netpay-form-installment">
                        <ul className="netpay-banks-list">
                        {
                                    ...
                                    <label htmlFor={backend['_id']}>
                                        ...
                                        <div className="bank-label">
                                            ...
                                            >
                                                <option>Select term</option>
                                                {
                                                    backend['available_plans'].map((installment_plan, i) => (
                                                        ...
                                                        >
                                                            {__(`${installment_plan['term_length']} months`, 'netpay')}
                                                            <>&nbsp;</>
                                                            ({__(`${installment_plan['monthly_amount']} / months`, 'netpay')})
                                                        </option>
                                                    ))
                                                }
                                            </select>
                                            {
                                                is_zero_interest && <>
                                                    <br />
                                                    <span className="netpay-installment-interest-rate">
                                                        {__( `( interest ${backend.interest_rate} )`, 'netpay' )}
                                                    </span>
                                                </>
                                            }
                                        </div>
