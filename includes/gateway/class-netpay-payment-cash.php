	...
	function netpay_cash_form() {
        if($this->is_test()) {
            ...
            <div id="netpay-installment-woocommerce-warning">
                <?php _e('- MODO PRUEBAS ACTIVADO. Si realizas transacciones no se enviará la referencia de pago al banco, y se enviará la referencia de pago de efectivo generada al correo electrónico que captures.', 'netpay'); ?>
            </div>
            <?php
        }
		...
		if($charge['result']['status'] == "success") {
			...
			$order->add_order_note(
				sprintf(
					...
					$charge['result']['paymentSource']['cashDto']['reference']
				)
			);
			...
			$order->add_order_note(
				sprintf(
					...
					$charge['result']['paymentSource']['cashDto']['expireInDays']
				)
			);
