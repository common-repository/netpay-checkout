...
class NetPay_Event_Charge_Complete extends NetPay_Event
{
	...
	public function resolve()
	{
		...
		$this->order->add_order_note(
			$this->allow_br('NetPay Payments: Received charge.complete webhook event.'));
	...
	private function handle_failed_charge()
	{
		...
		$failure_message = sprintf(
			'%s  (code: %s)',
			NetPay()->translate($this->data['failure_message']),
			$this->data['failure_code']
		);

		$this->order->add_order_note(
			sprintf(
				$this->allow_br('NetPay Payments: Payment failed.<br/>%s'),
				$failure_message
			)
		);

		$this->order->update_status(NetPay_Payment::STATUS_FAILED);
	}
	...
	private function handle_successful_charge()
	{
		...
		$this->order->add_order_note(
			sprintf(
				$this->allow_br('NetPay Payments: Payment successful.<br/>An amount %1$s %2$s has been paid'),
