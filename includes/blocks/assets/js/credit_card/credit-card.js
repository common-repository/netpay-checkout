...
const CreditCardPaymentMethod = (props) => {
	...
	useEffect(() => {
        const unsubscribe = onPaymentSetup(async () => {
			return await new Prnetpay( ( resolve, reject ) => {
					...
					if (savedCardIdRef.current && savedCardIdRef.current.value !== "") {
						...
						try {
							const response = {
								...
								meta: {
									paymentMethodData: {
										"netpay_save_customer_card": saveCardRef.current,
										"netpay_token": cardTokenRef.current,
									}
								}
							};
							resolve(response)
	...
	return (<>
		...
		<div ref={el} id="netpay-card" style={{width:"100%", display: hideCardForm ? "none" : "block"}}></div>
		{!hideCardForm && <>
			<input type="hidden" name="netpay_save_customer_card" id="netpay_save_customer_card" />
			<input type="hidden" className="netpay_token" name="netpay_token" />
		</>}
	</>)
}
