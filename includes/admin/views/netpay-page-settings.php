<div class="wrap netpay">
    ...
    <h2><?php echo _e( 'Configuraciones', 'netpay' ); ?></h2>

    <p>
        <?php
        echo _e( 'Las llaves las encontrarás en NetPay Manager (login requerido)', 'netpay' );
        ?>
    </p>
    ...
    <form method="POST">
        ...
        <table class="form-table">
            <tbody>
            <tr>
                <th scope="row"><label for="sandbox"><?php _e( 'Modo pruebas', 'netpay' ); ?></label></th>
                <td>
                    <fieldset>
                        <label for="sandbox">
                            <input name="sandbox" type="checkbox" id="sandbox" value="1" <?php echo 'yes' === $settings['sandbox'] ? 'checked="checked"' : ''; ?>>
                            <?php _e( 'Habilitar modo pruebas.', 'netpay' ); ?>
                        </label>
                    </fieldset>
                </td>
            </tr>
            <tr>
                <th scope="row"><label for="test_public_key"><?php _e( 'Llave pública para pruebas', 'netpay' ); ?></label></th>
                <td>
                    <fieldset>
            ...
            <tr>
                <th scope="row"><label for="test_private_key"><?php _e( 'Llave secreta para pruebas', 'netpay' ); ?></label></th>
                <td>
                    <fieldset>
            ...
            <tr>
                <th scope="row"><label for="live_public_key"><?php _e( 'Llave pública para producción', 'netpay' ); ?></label></th>
                <td>
                    <fieldset>
            ...
            <tr>
                <th scope="row"><label for="live_private_key"><?php _e( 'Llave secreta para producción', 'netpay' ); ?></label></th>
                <td>
                    <fieldset>
                                    ...
                                    foreach ( $columns as $key => $column ) :
                                        switch ( $key ) {
                                            case 'name' :
                                                ...
                                                echo '<td class="name">
															<a href="' . admin_url( 'admin.php?page=wc-settings&tab=checkout&section=' . strtolower( $gateway->id ) ) . '">' . ( $method_title ) . '</a>
														</td>';
                                                break;
