...
abstract class NetPay_Payment extends WC_Payment_Gateway {
    ...
    public function __construct() {
        ...
        add_action( 'woocommerce_order_status_processing', 'NetPayPluginHelperMailer::processing_admin_notification', 10, 2 );
        add_filter( 'woocommerce_email_recipient_new_order', 'NetPayPluginHelperMailer::disable_merchant_order_on_hold', 10, 2 );
    ...
    {
        if ( in_array( $metadataKeys, [ 'token', 'is_netpay_payment_resolved' ] )) {
            return true;
        }
    ...
    public function netpay_checkout_assets() {
        if ( is_checkout() ) {
            wp_enqueue_style( 'netpay', plugins_url( '../../assets/css/netpay-css.css', __FILE__ ), array(), NETPAY_WOOCOMMERCE_PLUGIN_VERSION );
    ...
    public function is_available(){
        ...
        $capabilities = NetPay_Capabilities::retrieve();
    ...
    /**
     * check if payment method is support by netpay capability api version 2017
     * 
    ...
    public function process_payment( $order_id ) {
        ...
        $this->order->add_order_note( sprintf( __( 'NetPay Payments: Processing a payment with %s', 'netpay' ), $this->method_title ) );
        ...
        $this->order->add_order_note( sprintf( __( 'NetPay Payments: Charge (ID: %s) has been created', 'netpay' ), $charge['id'] ) );
    ...
    public function process_capture( $order ) {
        ...
        try {
            ...
            $this->order()->add_order_note(
                sprintf(
                    wp_kses(
                        __( 'NetPay Payments: Payment successful (manual capture).<br/>An amount of %1$s %2$s has been paid', 'netpay' ),
                        array( 'br' => array() )
                    ),
            ...
            $netpayError = $e->getNetPayError();
            $this->order()->add_order_note(
                sprintf(
                    wp_kses( __( 'NetPay Payments: Capture failed (manual capture).<br/>%s', 'netpay' ), array( 'br' => array() ) ),
                    $e->getMessage()
                )
            );

            // we don't want to delete the capture metadata for other errors like 401, 403, and 500
            if ( self::ERROR_CODES['FAILED_CAPTURE'] === $netpayError['code'] || self::ERROR_CODES['EXPIRED_CHARGE'] === $netpayError['code'] ) {
                $this->delete_capture_metadata();
            }

            if ( self::ERROR_CODES['EXPIRED_CHARGE'] === $netpayError['code'] ) {
    ...
    public function process_refund( $order_id, $amount = null, $reason = '' ) {
        ...
        try {
            ...
            if ( $refund['voided'] ) {
                $message = sprintf(
                    wp_kses(
                        __( 'NetPay Payments: Voided an amount of %1$s %2$s.<br/>Refund id is %3$s', 'netpay' ),
                        array( 'br' => array() )
                    ),
                    $amount,
                    $order->get_currency(),
                ...
                $message = sprintf(
                    wp_kses(
                        __( 'NetPay Payments: Refunded an amount of %1$s %2$s.<br/>Refund id is %3$s', 'netpay' ),
                        array( 'br' => array() )
                    ),
                    $amount,
                    $order->get_currency(),
    ...
    protected function payment_failed( $reason ) {
        ...
        if ( $this->order() ) {
            $this->order()->add_order_note( sprintf( __( 'NetPay Payments: Payment failed, %s', 'netpay' ), $reason ) );
            $this->order()->update_status( 'failed' );
        }

        wc_add_notice( sprintf( wp_kses( $message, array( 'br' => array() ) ), __( $reason, 'netpay' ) ), 'error' );
    }
