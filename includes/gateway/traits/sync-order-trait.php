...
trait Sync_Order
{
    ...
    public function sync_payment($order)
    {
        ...
        try {
            ...
            switch ($charge['status']) {
                case NetPay_Payment::STATUS_SUCCESSFUL:
                    $this->handle_successful_charge($charge);
                    break;
                case NetPay_Payment::STATUS_FAILED:
                    $this->handle_failed_charge($charge);
                    break;
                case NetPay_Payment::STATUS_PENDING:
                    $this->handle_pending_charge();
                    break;
                case NetPay_Payment::STATUS_EXPIRED:
                    $this->handle_expired_charge();
                    break;
                case NetPay_Payment::STATUS_REVERSED:
                    $this->handle_reversed_charge();
                    break;
                default:
                    throw new Exception(
                        __('Cannot read the payment status. Please try sync again or contact NetPay Payments support team at support@netpay.co if you have any questions.', 'netpay')
                    );
                    break;
            }
        } catch (Exception $e) {
            $message = $this->allow_br('NetPay Payments: Sync failed (manual sync).<br/>%s.');
    ...
    {
        // NetPay API 2017-11-02 uses `refunded`, 
        // NetPay API 2019-05-29 uses `refunded_amount`.
        ...
        $this->order->add_order_note(
            sprintf(
                $this->allow_br('NetPay Payments: Payment successful.<br/>An amount %1$s %2$s has been paid (manual sync).'),
                $total_amount,
                $currency
            )
        );
    ...
    {
        $this->update_order_status(NetPay_Payment::STATUS_REFUNDED);
        $this->order->add_order_note(
            sprintf(
                $this->allow_br('NetPay Payments: Payment refunded.<br/>An amount %1$s %2$s has been refunded (manual sync).'),
                $amount,
                $currency
            )
        );
    ...
    private function handle_partially_refunded_charge($amount, $currency)
    {
        $this->order->add_order_note(
            sprintf(
                $this->allow_br('NetPay Payments: Payment partially refunded.<br/>An amount %1$s %2$s has been refunded (manual sync).'),
                NetPay_Money::convert_currency_unit($amount, $currency),
                $currency
            )
        );
    }
    ...
    private function handle_failed_charge($charge)
    {
        ...
        $this->order()->add_order_note(
            sprintf(
                $this->allow_br('NetPay Payments: Payment failed.<br/>%s (code: %s) (manual sync).'),
                NetPay()->translate($charge['failure_message']),
                $charge['failure_code']
            )
        );
        $this->update_order_status(NetPay_Payment::STATUS_FAILED);
    }
    ...
    private function handle_pending_charge()
    {
        $message = $this->allow_br(
            'NetPay Payments: Payment is still in progress.<br/>
            You might wait for a moment before click sync the status again 
            or contact NetPay Payments support team at support@netpay.co 
    ...
    private function handle_expired_charge()
    {
        $this->delete_capture_metadata();
        $this->order()->add_order_note(
            $this->allow_br('NetPay Payments: Payment expired. (manual sync).')
        );
        $this->update_order_status(NetPay_Payment::STATUS_CANCELLED);
    }
    ...
    private function handle_reversed_charge()
    {
        $this->delete_capture_metadata();
        $this->order()->add_order_note(
            $this->allow_br('NetPay Payments: Payment reversed. (manual sync).')
        );
        $this->update_order_status(NetPay_Payment::STATUS_CANCELLED);
    }
    ...
    {
        return wp_kses(__($message, 'netpay'), ['br' => []]);
    }
}
